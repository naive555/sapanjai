...
docs/            01-source-analysis · 02-api-contract · 03-target-architecture · 04-migration-plan ·
                 ...
                 08-gateway-core · 09-railway-deploy
...
- Redis keys: `blacklist:<accessToken>` (15 min), `login:attempts:<email>` (max 5 per 15 min), `worker:lock:<jobName>` (TTL ≈ job interval), `mcp:ratelimit:<connectorId>` (per-connector upstream-API token bucket, `MCP_RATE_LIMIT_PER_MIN` tokens/minute, idle TTL 2 min — `internal/infra/redis/ratelimit.go`, checked in `internal/module/mcp/service.go` before a `tools/call` dispatches).
