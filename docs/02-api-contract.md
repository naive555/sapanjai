...
| ---------------- | ------ | ---- | -------- |
| `POST /auth/register` | public | `{ email: email, password: 8-1024, displayName?: min 1 }` | `{ accessToken, refreshToken }` |
| `POST /auth/login`    | public | `{ email: email, password: max 1024 }` | `{ accessToken, refreshToken }` |
...
Recorded actions: `user.login`, `user.register`, `org.created`, `org.member.invited`, `org.member.removed`, `role.created`, `role.assigned` (last three defined but only the first four are currently written), plus `connector.created`, `connector.updated`, `connector.deleted` (all three written, from the Connectors module below), plus `mcp.session.started`, `mcp.tool.called`, `mcp.tool.denied`, `mcp.ratelimit.hit`, `mcp.file.downloaded` (all five written, from the MCP gateway below), plus `user.email_verified` (written by `POST /auth/verify-email`, from the Auth section above), plus `user.password_reset_requested` and `user.password_reset` (written by `POST /auth/forgot-password` and `POST /auth/reset-password` respectively, from the Auth section above), plus `billing.checkout.started`, `billing.portal.opened`, and `billing.subscription.synced` (written by `POST /billing/checkout`, `POST /billing/portal`, and `POST /billing/webhook` respectively, from the Billing section below).

### Subscription (`/subscription`)

| Method/Path | Guard | Body | Behavior |
| ----------- | ----- | ---- | -------- |
| `GET /subscription` | org | — | Org's subscription incl. plan (nullable if none): `{ id, organizationId, planId, customLimits, createdAt, updatedAt, plan: { id, name, limits, createdAt, prices: [] }, status, currentPeriodEnd, cancelAtPeriodEnd, hasActiveSubscription }`. `status`/`currentPeriodEnd`/`cancelAtPeriodEnd`/`hasActiveSubscription` were added by the billing plan's step 9 — see below for what each means. `plan.prices` is always `[]` here (see `GET /plans` below); the embedded plan answers "what plan is this org on", not "what could it buy instead". |
| `GET /plans` | auth | — | All public plans: `[{ id, name, limits, createdAt, prices: [{ unitAmount, currency, interval }] }]`. **Not in the source app.** Read-only catalogue: the subscription page shows which plan the org is on against the others it could subscribe to via `POST /billing/checkout` below. Global, not org-scoped, so `auth` not `org` guard. `prices` was added by billing plan step 9 — one entry per active `plan_prices` row for that plan; never carries `stripePriceId` (Stripe linkage has no business on a tenant-facing catalogue). Plans with `is_public = false` (superadmin-settable) are omitted entirely, since `POST /billing/checkout` refuses them and a listed-but-unbuyable plan would render a button that cannot work. |

**There is no tenant-facing plan write route.** The source app's `POST /subscription/assign` was removed: it sat on `RequireOrg`, which is membership-only and carries no role or permission check, so any org member — including a plain `member` — could put their own organization on any plan, raising `max_members`/`max_roles`/`max_connectors` at will. A plan is a commercial decision, not a tenant setting. The only way to change one is now `POST /admin/organizations/:orgId/plan` (platform:`superadmin`, see the Admin console section below) or a completed Stripe Checkout reconciled by `POST /billing/webhook` (see Billing below). The route is **gone, not denied** — a stale client gets the ordinary 404 `Route not found`, because the route table should not advertise a capability no tenant token can ever use.

### Billing (`/billing`)

Stripe Checkout/Portal integration and usage metering for the `.claude/plans/2026-09-13-billing-and-usage-metering.md` billing plan. `internal/module/billing`. Every route except the webhook sits on the caller's active organization (`x-organization-id`, resolved the same way every other org-scoped route resolves it) — never a body field, so there is no input through which a caller can act on an organization it is not a member of.

| Method/Path | Guard | Body | Behavior |
| ----------- | ----- | ---- | -------- |
| `POST /billing/checkout` | `billing:write` | `{ planId: uuid, interval?: "month"\|"year" }` | Starts a Stripe Checkout Session (`mode: "subscription"`) for the caller's org against `planId`, and returns `{ url }` — the browser redirects to it (`window.location.assign`; no Stripe.js, no embedded flow). Creates the org's Stripe Customer lazily on first use if it doesn't have one yet. `interval` defaults to `"month"`. Errors: `404 NOT_FOUND` (unknown or non-public `planId` — the two are indistinguishable on purpose), `409 PLAN_NOT_PURCHASABLE` (plan resolved but has no active `plan_prices` row for THB + the requested interval), `422 Validation failed`, `501 BILLING_NOT_CONFIGURED` (no `STRIPE_SECRET_KEY`), `502 BILLING_PROVIDER_ERROR` (Stripe call failed). |
| `POST /billing/portal` | `billing:write` | — | Opens a Stripe Customer Portal session for the caller's org (creating its Stripe Customer lazily if needed) and returns `{ url }` — same redirect-only shape as checkout. The Portal is where upgrade, downgrade, cancellation, payment-method changes, and invoice history all happen; carries the same guard as checkout, never a weaker one, since it can cancel a subscription and surfaces a billing address. Errors: `501 BILLING_NOT_CONFIGURED`, `502 BILLING_PROVIDER_ERROR`. |
| `GET /billing/usage` | `billing:read` | — | The caller's org's tool-call usage for the current UTC calendar month: `{ periodStart, periodEnd, callCount, limit, byTool: [{ tool, callCount }] }`. `callCount` is the **live** `usage_events` count — the same number `internal/module/mcp/service.go`'s quota check enforces against, not a `usage_rollups` sum, which can lag by up to `USAGE_ROLLUP_INTERVAL` for the still-open month. `limit` is `max_tool_calls_per_month` resolved through `subscription.Service.GetLimit` (plan limits overlaid by `custom_limits`); `null` means unlimited, covering both "plan omits the key" and "org has no subscription at all" the same way. `byTool` reads `usage_rollups` and is allowed to lag `callCount` — it's a breakdown, not the enforced number. Answers from Postgres alone; makes no Stripe call. |
| `POST /billing/webhook` | public (Stripe-Signature HMAC; optionally IP-allowlisted via `STRIPE_WEBHOOK_IP_ALLOWLIST`) | raw Stripe event | Reconciles `checkout.session.completed`, `customer.subscription.{created,updated,deleted}`, `invoice.paid`, `invoice.payment_failed` into `org_subscriptions` (calling `subscription.Service.AssignPlanTx`, never reimplementing the upsert) and returns `{ received: true }`. Idempotent via `stripe_events` (event id is the primary key); an out-of-order delivery is dropped rather than regressing state. Not mounted on the `billing:write`/`billing:read`-guarded group — Stripe presents no JWT and no `x-organization-id`. Errors: `400 WEBHOOK_SIGNATURE_INVALID` / `Invalid request body`, `501 BILLING_NOT_CONFIGURED` (no `STRIPE_WEBHOOK_SECRET`). An unhandled event type is dropped and answered `200` (Stripe retries a non-2xx for days and would eventually disable the endpoint). |

`billing:write` gates checkout and portal — both can change what the org pays for or manage its payment method, so both take the same guard (never `RequireOrg`, which is exactly the bug that got `POST /subscription/assign` deleted). `billing:read` gates only the usage view and is deliberately narrower: reading a call count cannot cancel a subscription or move money, so a finance person can be granted just this one action to watch the meter. Neither is owner-only — the RBAC engine's owner bypass covers an owner automatically, and either action can be granted to a non-owner without making them one.

Decision 3 (plan §Decisions): THB only, one active `plan_prices` row per plan per interval, Stripe Tax off. Decision 4: a Stripe Customer is created lazily on first billing interaction, never at org creation, and never with a zero-price Subscription for the free plan — `org_subscriptions.stripe_subscription_id IS NULL` is the canonical "not paying" signal, restated on `GET /subscription` as the boolean `hasActiveSubscription` rather than the raw id.
